...
func MigrationTest_CA_db_20241215165048_add_key_id(t *testing.T, logger *logrus.Entry, con *gorm.DB) {
	...
	assert.Equal(t, `{"type":"Duration","duration":"14w2d","time":""}`, caResult["issuance_expiration_ref"])
	assertEqualD(t, time.Date(2024, time.November, 25, 11, 45, 48, 620000000, time.UTC), caResult["creation_ts"].(time.Time))
	...
	assertEqualD(t, time.Time(time.Date(1, time.January, 1, 1, 0, 0, 0, time.UTC)), certResult["revocation_timestamp"].(time.Time))
