...
func NewCouchDMSRepository(couchURL url.URL, username, password string) (storage.DMSRepo, error) {
	client, err := createCouchDBConnection(couchURL, username, password, []string{dmsDB})
	if err != nil {
		return nil, err
	}
