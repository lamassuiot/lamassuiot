...
func TestWildcardSubscribe(t *testing.T) {
	testcases := []struct {
		...
		{
			name:            "OK/WildcardAtMiddle",
			subscriptionKey: "my/*",
			expectedTimeout: false,
		},
		// {
		// 	name:            "OK/WildcardAtRoot",
		// 	subscriptionKey: "*",
		// 	expectedTimeout: false,
		// },
		{
	...
	pub, err := NewEventBusPublisher(eventBusConf.config, "test-pub", helpers.SetupLogger(config.Trace, "Test Case", "pub"))
	if err != nil {
	...
	for _, tc := range testcases {
		...
		t.Run(tc.name, func(t *testing.T) {
			subAndPub := func(publishFunc func() error) error {
				...
				subHandler, err := NewEventBusSubscriptionHandler(eventBusConf.config, fmt.Sprintf("test-sub-%d", time.Now().UnixMilli()), logger, &basicTestHandler{
		...
		t.Run(tc.name, func(t *testing.T) {
			subAndPub := func(publishFunc func() error) error {
				...
				for _, sub := range tc.subscriptions {
					subHandler, err := NewEventBusSubscriptionHandler(eventBusConf.config, fmt.Sprintf("%s-%s-%d", tc.name, sub.serviceID, time.Now().UnixMilli()), logger, &basicTestHandler{
						handler: func(msg *message.Message) error {
	...
	for _, tc := range testcases {
		...
		t.Run(tc.name, func(t *testing.T) {
			subAndPub := func(publishFunc func() error) error {
				...
				for _, sub := range tc.subscriptions {
					subHandler, err := NewEventBusSubscriptionHandler(eventBusConf.config, fmt.Sprintf("%s-%s", tc.name, sub.serviceID), logger, &basicTestHandler{
						handler: func(msg *message.Message) error {
							t.Logf("subscriber %s - %s message ACK", sub.serviceID, sub.subscriptionKey)
							resultChannel <- sub.serviceID
							return nil
