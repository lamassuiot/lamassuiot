...
func TestSignVerify(t *testing.T) {
	tt := []struct {
	...
	}{
		{
			...
			testRestEndpoint: func(e *httpexpect.Expect) {
				_, message := generateBase64EncodedCertificateRequest("device-sign")
				reqBody := `{
					"message":"` + message + `",
					"message_type":"RAW",
					"signing_algorithm":"RSASSA_PSS_SHA_256 "
				}`
				obj := e.POST("/v1/pki/ca/ca-name-1/signature/sign").WithBytes([]byte(reqBody)).
					Expect().
					Status(http.StatusOK).JSON()
				signature := obj.Object().Value("signature").String().Raw()
				verifyBody := `{
						"message":"` + message + `",
						"message_type":"RAW",
						"signature":"` + signature + `",
				...
				verifyObj := e.POST("/v1/pki/ca/ca-name-1/signature/verify").WithBytes([]byte(verifyBody)).
					Expect().
					Status(http.StatusOK).JSON()
				verifyObj.Object().ContainsKey("result").ValueEqual("result", true)
			},
		},
	}
