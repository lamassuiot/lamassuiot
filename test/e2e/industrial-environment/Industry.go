...
var deviceCertFile = "./device.crt"
var deviceKeyFile = "./device.key"
...
func IndustrialEnvironment(caName string, deviceNumber int, reenroll int, certPath string, domain string) (string, error) {
	...
	dev, err := CreateDevices(devClient, deviceNumber, caName, dms.Id, reenroll, domain, dmsCert, dmsKey, serverCert, logger)
	if err != nil {
	...
	os.Remove(deviceCertFile)
	os.Remove(deviceKeyFile)
	return dms.Id, nil
}

func CreateDevices(devClient lamassudevice.LamassuDeviceManagerClient, deviceNumber int, caName string, dmsId string, reenroll int, domain string, clientCert *x509.Certificate, clientKey []byte, serverCert *x509.CertPool, logger log.Logger) (dto.Device, error) {
	var dev dto.Device
	var crt dto.Enroll
	...
	deviceKey, err := utils.ReadKey(deviceKeyFile)
	if err != nil {
		level.Error(logger).Log("err", err)
		return dto.Device{}, err
	}
