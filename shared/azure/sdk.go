...
func GetAzureCredential(conf AzureSDKConfig) (azcore.TokenCredential, error) {
	...
	case Secret:
		if conf.TenantID == "" || conf.ClientID == "" {
			return nil, fmt.Errorf("tenant ID and client ID are required for secret authentication method")
		}
	...
	case Certificate:
		if conf.TenantID == "" || conf.ClientID == "" {
			return nil, fmt.Errorf("tenant ID and client ID are required for certificate authentication method")
		}
