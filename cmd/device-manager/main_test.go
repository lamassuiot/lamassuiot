...
func TestDeviceHandler(t *testing.T) {
	...
	tt := []struct {
		...
		{
			name: "Simpleenroll_Unauthorized",
			serviceInitialization: func(s *service.Service) {
			},
			testRestEndpoint: func(s *httpexpect.Expect) {
			},
		...
		{
			name: "ServerKeyGen_Unauthorized",
			serviceInitialization: func(s *service.Service) {
			},
			testRestEndpoint: func(s *httpexpect.Expect) {
			},
			...
			testEstRestEndpoint: func(e *httpexpect.Expect) {
				...
				e.POST("/.well-known/est/CA/serverkeygen").WithHeader("X-Forwarded-Client-Cert", dmsCert).WithHeader("Content-Type", "application/pkcs10").WithBytes([]byte(
					reqBody.Bytes())).
					Expect().
					Status(http.StatusUnauthorized)
		...
		{
			name: "ServerKeyGen_ErrMalformedCert",
			serviceInitialization: func(s *service.Service) {
			},
			testRestEndpoint: func(s *httpexpect.Expect) {
			},
