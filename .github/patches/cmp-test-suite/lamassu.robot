...
# Lamassu's CMP device-enrollment issuance profile has HonorExtendedKeyUsages=true,
# so it issues certs carrying the CMP EKUs (cmcRA/cmcCA/cmKGA) requested in the
# CertTemplate rather than rejecting them. Enable the accept-path assertions.
...
# Device certificate/key from a FOREIGN PKI (not trusted by Lamassu), generated
# by run-cmp-test-suite.sh. Used by "Reject CR With Other PKI Management Entity"
# to confirm Lamassu rejects a signer that does not chain to its ValidationCAs.
${DEVICE_CERT_CHAIN}   /tmp/cmp-foreign/chain.pem
${DEVICE_KEY}  /tmp/cmp-foreign/device.key
...
# ${None} means "not provided": the CRL Update Retrieval tests correctly
# `Skip If '${CRL_CERT_IDP}' == 'None'` / '${CRL_FILEPATH}' == 'None'` when
# these are unset. A prior ${False} default here did not match that sentinel,
# so the positive test proceeded with cert=False and crashed with
# FileNotFoundError instead of skipping — we have no certificate carrying an
# Issuing Distribution Point extension to provide, so ${None} is correct.
${CRL_FILEPATH}    ${None}
${CRL_CERT_IDP}  ${None}
...
# Section 5.2 and 5.3
# Trusted PKI RA: a cert carrying id-kp-cmcRA issued by the bootstrap CA (which
# is in the DMS ValidationCAs), provisioned by cmp-bootstrap-setup.sh. Used both
# as the added-protection RA for nested PKIMessages (RFC 9483 §5.2.2) and as the
# trusted RA that may assert raVerified proof-of-possession (§5.2.3.2).
${OTHER_TRUSTED_PKI_KEY}    /tmp/cmp-bootstrap/ra.key
${OTHER_TRUSTED_PKI_CERT}    /tmp/cmp-bootstrap/ra-certs/ra.crt
${ALLOW_UNPROTECTED_INNER_MESSAGE}    ${None}
${RA_CERT_CHAIN_DIR}    /tmp/cmp-bootstrap/ra-certs
${RA_CERT_CHAIN_PATH}   ${None}

${RR_CERT_FOR_TRUSTED}   ${None}

# Cross-certification (ccr/ccp): a self-signed CA whose key the suite signs the
# ccr with. Lamassu authorizes a ccr by the requester being a CA (cA=TRUE), so a
# standalone self-signed CA is sufficient. These files are generated by
# scripts/run-cmp-test-suite.sh before the run.
${TRUSTED_CA_CERT}      /tmp/cmp-crosscert/certs/trusted-ca.crt
${TRUSTED_CA_KEY}       /tmp/cmp-crosscert/trusted-ca.key
${TRUSTED_CA_KEY_PASSWORD}   ${None}
${TRUSTED_CA_DIR}            /tmp/cmp-crosscert/certs
