...
func (s *SQLiteStorageEngine) initEventsStorage() error {
	...
	if s.StoredEvents == nil {
		s.StoredEvents, err = postgres.NewStoredEventsPostgresRepository(s.logger, s.db)
		if err != nil {
			return err
		}
	}
	if s.EventRetentionSettings == nil {
		s.EventRetentionSettings, err = postgres.NewEventRetentionSettingsPostgresRepository(s.logger, s.db)
		if err != nil {
			return err
		}
	}
...
func (s *SQLiteStorageEngine) GetStoredEventsStorage() (storage.StoredEventsRepository, error) {
	if s.StoredEvents == nil {
		if err := s.initEventsStorage(); err != nil {
			return nil, err
		}
...
func (s *SQLiteStorageEngine) GetEventRetentionSettingsStorage() (storage.EventRetentionSettingsRepository, error) {
	if s.EventRetentionSettings == nil {
		if err := s.initEventsStorage(); err != nil {
			return nil, err
		}
