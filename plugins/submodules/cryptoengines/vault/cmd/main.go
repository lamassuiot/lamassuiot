...
import (
	...
	vaultengine "github.com/lamassuiot/lamassuiot/v2/plugins/submodules/cryptoengines/vault"
...
func NewCryptoEngine(logger *logrus.Entry, config vaultengine.HashicorpVaultCryptoEngineConfig) (interfaces.CryptoEngine, error) {
	logger.Info("hello world")
	return NewVaultKV2Engine(logger, config)
}

func NewVaultKV2Engine(logger *logrus.Entry, conf vaultengine.HashicorpVaultCryptoEngineConfig) (interfaces.CryptoEngine, error) {
	var err error
	...
	httpClient, err := httputils.BuildHTTPClientWithTLSOptions(&http.Client{}, conf.TLSConfig)
	if err != nil {
		return nil, err
	}

	httpClient, err = httputils.BuildHTTPClientWithTracerLogger(httpClient, lVault)
	if err != nil {
		return nil, err
	}
