...
// and id-messageDigest (= SHA-256 over the eContent); and, per RFC 5652 §5.4, a
// signature computed over the DER of signedAttrs re-tagged as a UNIVERSAL SET
// OF — not the IMPLICIT [0] wire encoding, and not the EncapsulatedContentInfo.
func BuildSignedData(in SignedDataInput) ([]byte, error) {
	...
	attrsDER, err := marshalSignedAttrValues(in.EContentType, msgDigest[:])
	if err != nil {
		return nil, err
	}
