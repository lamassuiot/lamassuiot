...
func BuildSignedData(in SignedDataInput) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cms: marshal encapContentInfo: %w", err)
	}

	// signedAttrs: id-contentType (= EContentType) and id-messageDigest
	// (= SHA-256 over the eContent).
	msgDigest := sha256.Sum256(in.EContent)
	signedAttrsImplicit, err := buildSignedAttrs(in.EContentType, msgDigest[:])
	if err != nil {
		return nil, err
	}

	// Signature: over the encapContentInfo DER (RFC 9483 §4.1.6 profile).
...
// SET OF Attribute carrying id-contentType (= eContentType) and id-messageDigest.
func buildSignedAttrs(eContentType asn1.ObjectIdentifier, messageDigest []byte) ([]byte, error) {
