...
// AsymmetricKeyPackage (SEQUENCE OF OneAsymmetricKey) wrapper this function
// emits, even though the signature and encoding are independently verified
// correct (see cms_test.go / cmp_kga_crypto_test.go, which recompute the
// RFC 5652 §5.4 digest exactly like a real verifier). The wrapper is not
// optional: RFC 9483 §4.1.6 and the Siemens cmp-test-suite this DMS is
// validated against both require it, and the suite passes all KGA tests
// against this exact response shape. Do NOT drop the AsymmetricKeyPackage
// wrapper to accommodate openssl — that would trade a documented,
// unfixable-on-our-side CLI gap for genuine RFC/suite non-compliance.
// Operators needing openssl-cmp-driven CKG need a client whose CKG
// implementation accepts the RFC-mandated wrapper.
