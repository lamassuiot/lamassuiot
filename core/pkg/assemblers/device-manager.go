...
import (
	...
	"github.com/lamassuiot/lamassuiot/v2/core/pkg/middlewares/eventpub"
	"github.com/lamassuiot/lamassuiot/v2/core/pkg/models"
	"github.com/lamassuiot/lamassuiot/v2/core/pkg/routes"
	"github.com/lamassuiot/lamassuiot/v2/core/pkg/services"
	"github.com/lamassuiot/lamassuiot/v2/core/pkg/storage"
	"github.com/lamassuiot/lamassuiot/v2/core/pkg/storage/couchdb"
	"github.com/lamassuiot/lamassuiot/v2/core/pkg/storage/postgres"
...
func AssembleDeviceManagerService(conf config.DeviceManagerConfig, caService services.CAService) (*services.DeviceManagerService, error) {
	...
	if conf.EventBus.Enabled {
		...
		updateCertStatusHandler := func(msg *message.Message) {
			...
			cert, err := messaging.GetEventData[models.UpdateModel[models.Certificate]](event)
			if err != nil {
		...
		updateCertMetaHandler := func(msg *message.Message) {
			...
			certUpdate, err := messaging.GetEventData[models.UpdateModel[models.Certificate]](event)
			if err != nil {
				lMessaging.Errorf("could not decode cloud event: %s", err)
