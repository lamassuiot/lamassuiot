...
import (
	...
	"github.com/lamassuiot/lamassuiot/v2/core/pkg/x509engines"
...
type Engine struct {
	Default bool
	Service interfaces.CryptoEngine
}

type CAServiceImpl struct {
	service               CAService
	cryptoEngines         map[string]*interfaces.CryptoEngine
	defaultCryptoEngine   *interfaces.CryptoEngine
...
func NewCAService(builder CAServiceBuilder) (CAService, error) {
	...
	engines := map[string]*interfaces.CryptoEngine{}
	var defaultCryptoEngine *interfaces.CryptoEngine
...
func (svc *CAServiceImpl) ImportCA(ctx context.Context, input ImportCAInput) (*models.CACertificate, error) {
	...
	if input.CAType != models.CertificateTypeExternal {
		...
		var engine interfaces.CryptoEngine
		if input.EngineID == "" {
