...
import (
	...
	lamassu "github.com/lamassuiot/lamassuiot/v2/core/pkg/assemblers"
...
func RunMonolithicLamassuPKI(conf config.MonolithicConfig) (int, error) {
	...
	if conf.AssemblyMode == config.Http {
		...
		caHttpCli, err := sdk.BuildHTTPClient(config.HTTPClient{
		...
		devMngrHttpCli, err := sdk.BuildHTTPClient(config.HTTPClient{
			LogLevel:       config.Info,
			AuthMode:       config.NoAuth,
		...
		// dmsMngrHttpCli, err := sdk.BuildHTTPClient(config.HTTPClient{
		// 	LogLevel:       config.Info,
		// 	AuthMode:       config.NoAuth,
