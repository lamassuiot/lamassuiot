...
type ESTMiddleware func(services.ESTService) services.ESTService

type ESTBackendBuilder struct {
	CAService      services.CAService
	DeviceManager  services.DeviceManagerService
	DMSManager     services.DMSManagerService
	Logger         *logrus.Entry
...
type ESTBackend struct {
	...
	devManager     services.DeviceManagerService
	dmsManager     services.DMSManagerService
	logger         *logrus.Entry
	downstreamCert *x509.Certificate
}

func NewESTForRA(builder ESTBackendBuilder) services.ESTService {
	return &ESTBackend{
		dmsManager:     builder.DMSManager,
		caClient:       builder.CAService,
		devManager:     builder.DeviceManager,
...
func (svc ESTBackend) checkCertificateRevocation(ctx context.Context, cert *x509.Certificate, validationCA *x509.Certificate) (bool, bool, error) {
...
func (svc ESTBackend) CACerts(ctx context.Context, aps string) ([]*x509.Certificate, error) {
	lFunc := chelpers.ConfigureLogger(ctx, svc.logger)

	cas := []*x509.Certificate{}
	lFunc.Debugf("checking if DMS '%s' exists", aps)
	dms, err := svc.dmsManager.GetDMSByID(ctx, services.GetDMSByIDInput{
		ID: aps,
	})
	if err != nil {
		lFunc.Errorf("aborting. Could not get DMS '%s': %s", aps, err)
		return nil, err
	}
...
func (svc ESTBackend) Enroll(ctx context.Context, csr *x509.CertificateRequest, aps string) (*x509.Certificate, error) {
	lFunc := chelpers.ConfigureLogger(ctx, svc.logger)
	...
	device, err = svc.devManager.GetDeviceByID(ctx, services.GetDeviceByIDInput{
	...
	if dms.Settings.EnrollmentSettings.RegistrationMode == models.JITP {
		if device == nil {
			...
			device, err = svc.devManager.CreateDevice(ctx, services.CreateDeviceInput{
				ID:        csr.Subject.CommonName,
	...
	_, err = svc.dmsManager.BindIdentityToDevice(ctx, services.BindIdentityToDeviceInput{
		DeviceID:                device.ID,
...
func (svc ESTBackend) Reenroll(ctx context.Context, csr *x509.CertificateRequest, aps string) (*x509.Certificate, error) {
	lFunc := chelpers.ConfigureLogger(ctx, svc.logger)
...
func (svc ESTBackend) ServerKeyGen(ctx context.Context, csr *x509.CertificateRequest, aps string) (*x509.Certificate, any, error) {
	lFunc := chelpers.ConfigureLogger(ctx, svc.logger)
