...
func SendNotification(ctx context.Context, c models.Channel, smtpServer config.SMTPServer, event cloudevents.Event) error {
	svc, err := GetOutputService(c, smtpServer)
	if err != nil {
		return err
	}
	return svc.SendNotification(ctx, event)
}
