...
func (r *cmpHttpRoutes) handleEnrollment(ctx *gin.Context, lFunc *logrus.Entry, header corecmp.RequestPKIHeader, body asn1.RawValue, dmsID string, enrollOpts *models.EnrollmentOptionsLWCRFC9483, variant enrollmentVariant, signerCert *x509.Certificate) {
	...
	// fixed protocol invariant, not something gated by EnforcePOPO, so there
	// is nothing left to check at this point.
...
//
// The key pair is generated with an in-process software engine
// (generateKGAKey), not this DMS's configured KMS/HSM engine: every other key
// in this platform is generated by, and never leaves, its KMS/HSM engine
// (exposed only as an opaque crypto.Signer), but CKG's entire purpose is to
...
func (r *cmpHttpRoutes) handleKGAEnrollment(
	...
	// CKG hands the RA-generated private key over to whoever "recipient"
	// names; that is a materially bigger trust decision than plain issuance,
	// so it is validated unconditionally here — regardless of the DMS's
	// AuthMode (see LightweightCMPKGARecipientValidator/
	// CKGTrustedEncryptionCAs). A service that does not implement this
	// capability cannot prove the recipient is trusted, so CKG is refused
	// rather than silently generating and delivering key material to an
	// unvalidated certificate.
	kgaValidator, ok := r.svc.(services.LightweightCMPKGARecipientValidator)
	if !ok {
		lFunc.Errorf("kga: service does not implement LightweightCMPKGARecipientValidator")
	...
	if vErr := kgaValidator.LWCValidateKGARecipient(ctx.Request.Context(), dmsID, recipient); vErr != nil {
		lFunc.Warnf("kga: recipient certificate is not trusted for this DMS: %v", vErr)
		r.rejectCertRequest(ctx, lFunc, header, respTag, dmsID, &corecmp.CertRequestRejection{
			CertReqID:   req.CertReqID,
			Reason:      "central key generation recipient certificate is not trusted by this DMS",
			FailInfoBit: corecmp.PKIFailureInfoSignerNotTrusted,
		})
		return
	}
	...
	// ir/cr/p10cr) applies CR-only settings (RequireExistingDevice,
	// MaximumActiveCertificates, CertificateBehavior, AllowedProfileIDs)
	// instead of silently falling back to ir's policy for a cr-tagged KGA
	// request (cmpOperationFromContext defaults to "ir" when this is unset).
	issuanceCtx = context.WithValue(issuanceCtx, core.LamassuContextKeyCMPOperation, cmpTagToString(requestTag))
