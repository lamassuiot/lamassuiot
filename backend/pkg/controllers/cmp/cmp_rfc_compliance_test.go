...
func buildHeaderDERCustom(t *testing.T, o headerOpts) []byte {
	...
	if !o.OmitMessageTime {
		content = append(content, messageTimeFieldDER(t, time.Now())...)
	}
...
func parseResponseHeaderMessageTime(t *testing.T, responseDER []byte) time.Time {
	t.Helper()
	f, ok := responseHeaderField(t, responseDER, 0)
	if !ok {
		return time.Time{}
	}
	var ts time.Time
...
func parseResponseSenderKID(t *testing.T, responseDER []byte) []byte {
	...
	return responseHeaderOctetString(t, responseDER, 2)
}
...
func extractResponseRecipNonce(t *testing.T, responseDER []byte) []byte {
	t.Helper()
	return responseHeaderOctetString(t, responseDER, 6)
}
...
// trip the outer asn1.Unmarshal. There is no valid PKIHeader to echo the
// request's transactionID / senderNonce, so a conformant CMP error body cannot
// be built; per RFC 6712 §3.3 the server answers with HTTP 400 instead.
func TestFailInfo_MalformedPKIMessage_BadDataFormat(t *testing.T) {
	...
	require.Equal(t, http.StatusBadRequest, resp.Code,
		"unparseable PKIMessage MUST yield HTTP 400 per RFC 6712 §3.3")
	require.Empty(t, resp.Body.Bytes(),
		"HTTP 400 for garbage input carries no CMP body")
}
...
func TestFailInfo_AllErrorResponsesCarryFailInfo(t *testing.T) {
	...
	t.Run("garbage body → HTTP 400", func(t *testing.T) {
		svc := &cmpmock.MockLightweightCMPService{}
		router, _ := newTestRouterWithStore(svc)
		resp := postCMP(t, router, "test-dms", []byte{0xFF, 0xFE})
		// Unparseable input has no header to echo, so it cannot be answered with
		// a conformant error PKIMessage — RFC 6712 §3.3 mandates HTTP 4xx instead.
		assert.Equal(t, http.StatusBadRequest, resp.Code,
			"garbage body MUST yield HTTP 400, not a CMP error body")
	})
...
func parseResponseSenderNonce(t *testing.T, responseDER []byte) []byte {
	t.Helper()
	return responseHeaderOctetString(t, responseDER, 5)
}
