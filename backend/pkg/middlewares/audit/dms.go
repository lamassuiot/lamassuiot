...
func NewDmsAuditEventPublisher(audit eventpublisher.AuditPublisher) lservices.DMSManagerMiddleware {
	return func(next services.DMSManagerService) services.DMSManagerService {
		return &DmsAuditEventPublisher{
			next: next,
			auditPub: eventpublisher.AuditPublisher{
				ICloudEventPublisher: eventpublisher.NewEventPublisherWithSourceMiddleware(audit, models.DMSManagerSource),
			},
		}
	}
}
